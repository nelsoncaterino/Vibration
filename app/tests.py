...
def test_evenement_admin_registered():
    ...
    assert test_evenement_admin_registered.is_registered(Evenement), "Le modèle Evenement n'est pas enregistré dans l'administration"

@pytest.mark.django_db
def test_visitor_admin_registered():
    # Vérifier que le modèle Visitor est bien enregistré dans l'admin
    assert test_visitor_admin_registered.is_registered(Visitor), "Le modèle Visitor n'est pas enregistré dans l'administration"

@pytest.mark.django_db
def test_playedtrack_admin_registered():
    # Vérifier que le modèle PlayedTrack est bien enregistré dans l'admin
    assert test_playedtrack_admin_registered.is_registered(PlayedTrack), "Le modèle PlayedTrack n'est pas enregistré dans l'administration"
